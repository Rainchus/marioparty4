...
A work-in-progress decompilation of Mario Party 4. While the two USA versions are almost completely matching (the only non-matching function has a small difference which doesn't have an impact on further work), most non-game-engine code is not documented at all. Work on the PAL and JP versions is currently stale, as it involves a lot of repetitive work, which could be done much easier later when there is special tooling for porting splits and symbols between versions.
